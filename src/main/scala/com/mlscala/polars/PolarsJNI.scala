...
object PolarsJNI {
  ...
  def createSampleDataFrameSafe(): Either[String, String] = {
    Try {
      ensureLibraryLoaded()
  ...
  def readCsvSafe(path: String): Either[String, String] = {
    Try {
      ensureLibraryLoaded()
  ...
  def filterDataFrameSafe(csvPath: String, column: String, minValue: Double): Either[String, String] = {
    Try {
      ensureLibraryLoaded()
  ...
  def groupByAndSumSafe(csvPath: String, groupCol: String, sumCol: String): Either[String, String] = {
    Try {
      ensureLibraryLoaded()
      if (!Files.exists(Paths.get(csvPath))) {
        throw new IllegalArgumentException(s"File not found: $csvPath")
