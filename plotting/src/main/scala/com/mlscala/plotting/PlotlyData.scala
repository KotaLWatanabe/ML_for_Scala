...
object ChartType:
  ...
  inline given Encoder[ChartType] = Encoder.instance {
...
object TraceConfig:
  inline given Encoder[TraceConfig] = deriveEncoder
...
object Trace:
  inline given Encoder[Trace] = deriveEncoder

  inline def scatter(points: List[Point], config: TraceConfig = TraceConfig()): Trace =
    Trace(
      x = points.map(_.x),
      y = points.map(_.y),
  ...
  inline def line(points: List[Point], config: TraceConfig = TraceConfig()): Trace =
    Trace(
      x = points.map(_.x),
      y = points.map(_.y),
      `type` = ChartType.Line,
  ...
  inline def bar(x: List[Double], y: List[Double], config: TraceConfig = TraceConfig()): Trace =
    Trace(
      x = x,
      y = y,
      `type` = ChartType.Bar,
...
object Layout:
  inline given Encoder[Layout] = deriveEncoder

  inline def withTitle(title: String): Layout = Layout(title = Some(title))
...
object PlotData:
  inline given Encoder[PlotData] = Encoder.instance { plotData =>
    Json.obj(
