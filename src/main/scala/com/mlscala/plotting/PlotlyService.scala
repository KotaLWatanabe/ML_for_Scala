...
object PlotDataGenerator:
  ...
  def generateRandomPoints(count: Int, xRange: (Double, Double) = (0.0, 10.0)): IO[List[Point]] =
    IO {
  ...
  def generateSinWave(points: Int = 100, amplitude: Double = 1.0, frequency: Double = 1.0): IO[List[Point]] =
    IO {
  ...
  def generateLinearData(slope: Double = 1.0, intercept: Double = 0.0, noise: Double = 0.0): IO[List[Point]] =
    IO {
      val random = new Random()
      (0 to 50).map { i =>
        val x = i.toDouble
