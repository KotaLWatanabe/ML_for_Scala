...
object DataFrameOps {
  ...
  def createSampleDataFrame(): IO[Either[String, DataFrameResult]] = {
    IO {
      PolarsJNI.createSampleDataFrameSafe().flatMap(parseDataFrameJson)
    }
  }

  def readCsvFile(path: String): IO[Either[String, DataFrameResult]] = {
    IO {
      PolarsJNI.readCsvSafe(path).flatMap(parseDataFrameJson)
    }
  }

  def filterDataFrame(csvPath: String, column: String, minValue: Double): IO[Either[String, DataFrameResult]] = {
    IO {
      PolarsJNI.filterDataFrameSafe(csvPath, column, minValue).flatMap(parseDataFrameJson)
    }
  }

  def groupByAndSum(csvPath: String, groupCol: String, sumCol: String): IO[Either[String, DataFrameResult]] = {
    IO {
      PolarsJNI.groupByAndSumSafe(csvPath, groupCol, sumCol).flatMap(parseDataFrameJson)
    }
  }

  private def parseDataFrameJson(jsonStr: String): Either[String, DataFrameResult] = {
